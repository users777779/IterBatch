...
2025-08-12 (改进版本)
- 移除硬门槛：删除 acc >= 0.7 的绝对禁用，改为相对门槛 acc_gap < -0.02。
- 收紧后期阈值：loss_ratio 从 1.2/1.1 降到 1.08/1.03，acc 条件改为 0.99/0.995。
- 历史窗口持久化：recent_losses/accs 不再每个 epoch 重置，改为持久化存储。
- 降低历史门槛：从 10 降到 5，减少早期训练的不触发。
- 保留探索：epsilon 最小值设为 0.02，避免完全归零。
- 自适应阈值：当 label_1 < 10% 或 action_1 < 5% 时自动放宽阈值。
- 支持误分类重复：可选只对误分类样本进行重复训练（提升梯度利用率）。

2025-08-12 (激进改进版本)
- 大幅提高重复训练上限：从30%提升到50%，解决0.299固定值问题。
- 激进阈值调整：loss_ratio从1.08/1.03降到1.02/1.01，acc从0.99/0.995降到0.995/0.998。
- 新增激进规则：只要loss>avg_loss或acc<avg_acc就触发重复训练。
- 放宽相对门槛：acc_gap从-0.02放宽到-0.005。
- 提升重复训练强度：学习率从0.8x提升到1.2x。
- 实现自适应阈值：当触发率过低时自动启用更宽松的阈值。
- 目标：大幅提高触发率，让决策网络真正发挥作用。
